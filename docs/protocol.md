...
```
命令: IPMSG_SENDIMAGE (0xC0)
选项: IPMSG_FILEATTACHOPT (0x00200000)
附加区: 8字节图片ID
```
...
class RecvImage : public RecvProtocol {
    bool read(shared_ptr<Post> post) {
        ...
        {
            char id[9] = {0};
            memcpy(id, post->extra.data(), 8);  // 前8字节为图片ID
...
> ⚠️ **本项目状态**：接收到图片消息时会自动回复 "Mac飞秋还不支持接收图片，请用文件形式发送图片"。
...
| MAC 地址识别 | ✅ 完整 | macOS 通过 en0 获取 |
| 消息确认机制 | ✅ 完整 | SENDCHECK + 5秒超时 |
| 已读回执 | ✅ 完整 | READCHECK |
| 自定义广播组 | ✅ 完整 | 定时发送上线通知 |
...
| 功能 | 状态 | 说明 |
|:---|:---|:---|
| 图片接收 | ⚠️ 协议解析完成 | 收到后回复"不支持" |
| 公钥交换 | ⚠️ 空壳类 | `GetPubKey` 已定义但无逻辑 |
| 聊天记录 | ⚠️ 半成品 | SQLite 表已建，`getFellow`/`findFellowId` 未实现 |

### 未实现

| 功能 | 协议支持 | 说明 |
|:---|:---|:---|
| 目录传输 | ✅ IPMSG 标准 | 需要递归目录结构解析 |
| 离开/忙碌状态 | ✅ BR_ABSENCE | 协议层有定义，业务层未使用 |
| 自动应答 | ✅ AUTORETOPT | 协议层有定义，完全未实现 |
